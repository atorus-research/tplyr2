...
(preferred term) appears indented in `rowlabel2`. Outer rows contain aggregate
counts for the body system; inner rows contain per-term counts. Indentation
defaults to two spaces and can be changed via the `indentation` parameter in
`layer_settings()`.
...
- `vignette("pop_data")` -- Population data, header N, total groups, custom
  groups.
- `vignette("post_processing")` -- Row masks, column headers, row label
  collapsing.
- `vignette("metadata")` -- Cell-level metadata and traceability.
- `vignette("serialize")` -- Saving and loading specs as JSON or YAML.
