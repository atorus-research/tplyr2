...
When a `by` is a list mixing data variables and labels, each label
*element* serializes with the singular key `value` instead. Regular data
variable names (character strings) pass through as-is.
...
`_fn`. In particular, all of the 0.2.0 comparative-statistics and
